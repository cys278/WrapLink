...
describe("PostgresLinkStore", () => {
  it("creates and retrieves a link", async () => {
    ...
    const found = await store.find("docs");

    expect(found).toEqual(created);
    expect(await store.size()).toBe(1);
  });

  it("returns null when a code already exists", async () => {
    const input = {
      code: "same",
  ...
  it("increments click counts atomically", async () => {
    await store.create({
      code: "clicks",
      targetUrl: "https://example.com",
      expiresAt: null,
    ...
    await Promise.all(
      Array.from({ length: 25 }, () => store.recordClick("clicks")),
    );

    const found = await store.find("clicks");
